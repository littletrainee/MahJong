...
func (h *Handler) update(p1, p2, p3, p4 *Player.Player) string {
	...
	if !p1.Iswin.Get() {
		...
		if p4.Iswin.Get() {
			...
			h.wg.Wait()
			// if p4 has pong or kang meld then ask make pong or kang meld
			if p4.HasKangMeld.Get() || p4.HasPongMeld.Get() {
				choice := p4.AskMakeMeldAndChoose()
				switch choice {
				case "p": // select is pong
					p4.MakePongMeld(p1)
					p4.ResetMeld() // Reset Any Kind Meld
					goto SetTargetToThisPlayerToDiscardByp4
				case "k": // select is kang
					p4.MakeBigKangMeld(p4, &h.Wall)
					if p4iswin := repeatcheckkangwithwin(p4, p1, h); p4iswin == "" { // check p3 is win
						p4.ResetMeld()                          // Reset Any Kind Meld
						goto SetTargetToThisPlayerToDiscardByp4 // not win then goto SetTargetPlayer label
					} else {
						return p4iswin
					}
			...
			SetTargetToThisPlayerToDiscardByp4:
				h.GameState.TurnTargetPlayer(p4.CodeNumber.Get())
				goto ReturnNoWinner
			}
		NextPlayerByp3:
			// if p3 has pong or kang meld then ask make pong or kang meld
			if p3.HasKangMeld.Get() || p3.HasPongMeld.Get() {
				...
				case "p": // select is pong
					...
					goto SetTargetToThisPlayerToDiscard
				case "k": // select is kang
					p3.MakeBigKangMeld(p1, &h.Wall)
					if p3iswin := repeatcheckkangwithwin(p3, p1, h); p3iswin == "" { // check p3 is win
						p3.ResetMeld()                      // Reset Any Kind Meld
						goto SetTargetToThisPlayerToDiscard // not win then goto SetTargetPlayer label
					} else {
						return p3iswin
					}
				case "s": // skip to nextplayer
			...
			SetTargetToThisPlayerToDiscard:
				h.GameState.TurnTargetPlayer(p3.CodeNumber.Get())
				goto ReturnNoWinner
			}
			// if p3 and p4 hasn't pong or kang meld
		NextPlayer:
			if !p2.TenPai.Get() {
